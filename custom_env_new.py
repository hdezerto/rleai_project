...
class Joystick(go1_base.Go1Env):
  ...
  def __init__(
      ...
      max_wall_height: float = 0.4,
      start_wall_height: float = -0.05,
      difficulty_slope: float = 0.01,
      stddev: float = 0.01,
  ):
    ...
    self._post_init()
    self._max_wall_height = max_wall_height
    self._start_wall_height = start_wall_height
    self._difficulty_slope = difficulty_slope
    self._stddev = stddev
  ...
  def sample_wall_heights(self, wall_rng, mean_height):
    """Sample random wall heights (JAX-compatible)."""
    num_walls = len(self._wall_geom_ids)
    random_values = jax.random.normal(wall_rng, shape=(num_walls,))
    wall_heights = mean_height + random_values * self._stddev
    wall_heights = jp.clip(wall_heights, self._start_wall_height, self._max_wall_height)

    return wall_heights, wall_rng

  # NOTE: Here we added an example to understand how to use the episode tracker in custom_ppo_train.py
  def _get_current_target_height(self, episode: jax.Array) -> float:
    target_height = self._start_wall_height + (episode * self._difficulty_slope)
    effective_height = jp.minimum(target_height, self._max_wall_height)
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    # NOTE: We cannot pass in more info during reset, so we will assume 0.
    episode = jp.array(0)
    target_height = self._get_current_target_height(episode)
    wall_heights, wall_rng = self.sample_wall_heights(wall_rng, target_height)
  ...
  # NOTE: The step function now handles wall height setup, since we inform about the resets in the state's info.
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    
    # ========================= Handle wall height updates ======================
    # Access reset info:
    episode_just_reset = state.info.get('episode_just_reset', jp.array(False))
    was_truncation = state.info.get('reset_was_truncation', jp.array(False))
    current_episode = state.info.get('episode', jp.array(0))
    
    data = state.data

    
    succeeded = episode_just_reset & was_truncation  # Then we update!
    any_success = jp.any(succeeded)

    # Single conditional update
    # MUST return both data and new_rng to avoid side effects
    def update_walls_for_successes(data_and_rng):
        data, rng = data_and_rng
        target_height = self._get_current_target_height(current_episode)

        rng, wall_rng = jax.random.split(rng)
        wall_heights, rng = self.sample_wall_heights(wall_rng, target_height)
        
        updated_data = self.set_wall_mocap_positions(data, wall_heights)
        return updated_data, rng
    
    def keep_walls(data_and_rng):
        return data_and_rng
    
    # Conditional update based on any success
    data, new_rng = jax.lax.cond(
        any_success, 
        update_walls_for_successes, 
        keep_walls, 
        (data, state.info["rng"])
    )
    
    state.info["rng"] = new_rng
    
    state = state.replace(data=data)
    state.info["episode_just_reset"] = jp.array(False, dtype=bool) # we handled it
